...
/*
    ...
    @param buff: buffer donde almacenar los bytes leidos del fichero
    @param size: numero de bytes a leer
    @param offset: ubicacion del puntero de posicion del fichero abierto
    @param fi: estructura que representa al fichero abierto en FUSE

    @return >0 on success (showing the amout of bytes read), 0 if EOF and <0 on error
*/
static int my_read(const char * path, char * buff, size_t size, off_t offset, struct fuse_file_info * fi){

    char buffer[BLOCK_SIZE_BYTES]; //Creamos un buffer para copiar el contenido de un bloque
    int bytes2read = size; //Numero de bytes que vamos a leer
    int totalRead = 0; //Numero de bytes que llevamos leidos
    NodeStruct *node = myFileSystem.nodes[fi->fh]; //Guardamos el inodo asociado al fichero a leer

    fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

    //En cada iteracion del bucle, vamos leyendo un bloque del fichero y lo guardamos en
    // el buffer privado de la funcion.
    //De ese buffer privado debemos ir copiando al buffer pasado por parametro.
    while(bytes2read) {
        int i;
        int currentBlock, offBlock;
        currentBlock = node->blocks[offset / BLOCK_SIZE_BYTES];
    ...
    TODO preguntar si esto es necesario; yo creo que no, puesto que no modificamos nada del sistema de ficheros,
    solo estamos leyendo datos.

    sync(); //Vacia la cache de bloques, copiando los bloques resultantes a disco, para evitar que
            //quede informacion sin copiar.
...
static int my_unlink(const char * path){
    ...
    PSEUDOCODIGO:
    Acciones my_unlink():
        * Recuperar structuras i-nodo
            - Buscar fichero en directorio ==> obtenemos índice de entrada (el parametro *path viene con /a.txt, buscar una funcion que quita la barra y devuelve la entrada de directorio y accediendo a esa entrada de directorio sabemos la clave del i-nodo y recuperar la estrucutra de ese i-nodo para poder empezar a trabajar)
            - Recuperar número de i-nodo (k) de entrada de directorio
        * Poner bloque que usa el fichero a 0 en Bitmap
        * Actualizar superbloque
        * Liberar i-nodo (abajo)

    para quitar la barra esa de /a.txt hacer, por ejemplo (cuando comparamos cadenas: strcmp(path + 1))


    Liberar i-nodo:
        * Ponerlo como libre (freeNode = 1)
        * Escribir nodo en disco (funcion que viene predefinida ==> preguntar)
        * Poner entrada k-esima de array de i-nodos a null

    Hacerlo con todas las estructuras que usan estos i-nodos en memoria*/

    // Si el nombre que pasas es más largo que el maximo permitido, no se puede borrar pues no está
    if (strlen(path + 1) > myFileSystem.superBlock.maxLenFileName)
        return -ENAMETOOLONG;
    ...
    int idxBlock, count = 0;

    // Recorremos los bloques que tiene archivo para ir poniendolos a 0 (libres)
    for (int i = 0; i < myFileSystem.nodes[idxFile]->numBlocks; i++) {
        idxBlock = myFileSystem.nodes[idxFile]->blocks[i];
        myFileSystem.bitMap[idxBlock] = 0;
        count++;
    }

    // Actualizamos la informacion del superbloque
    myFileSystem.directory.numFiles -= 1;
    myFileSystem.directory.files[idxFile].freeFile = true;
    myFileSystem.numFreeNodes += 1;

    // Actualizo el numero de bloques libre de nuestro SF
    myFileSystem.superBlock.numOfFreeBlocks += count;

    // Pongo en disco el i-nodo del archivo a borrar a 1 (como libre) y en memoria lo pongo a NULL
    myFileSystem.nodes[idxFile]->freeNode = 1;
    myFileSystem.nodes[idxFile] = NULL;

    //Ahora actualizamos el backup (virtual disk)

    updateSuperBlock(&myFileSystem);
    updateBitmap(&myFileSystem);
    updateDirectory(&myFileSystem);

    NodeStruct * nnn = malloc(sizeof(NodeStruct));
    nnn->numBlocks = count;
    nnn->fileSize = 0;
    nnn->modificationTime = time(NULL);
    //TODO habria que poner algo en el array DISK_LBA blocks???? en plan inicializarlo a algo
    nnn->freeNode = 1;

    updateNode(&myFileSystem, idxFile, nnn);
...
struct fuse_operations myFS_operations = {
    ...
    .read       = my_read,                      // Read the content of a file
